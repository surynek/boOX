...
/*----------------------------------------------------------------------------*/
	...
	*/
	...
	{ /* kruhobots are colliding, calculating time shift t0 */
	    ...
	    {
		sDouble lower_unsafe_A, upper_unsafe_A;
		sDouble t0_beta, t0_gamma_1, t0_gamma_2;

		if (sABS(beta_1) > s_EPSILON)
		{
		    t0_beta = -beta_0 / beta_1;
		    sDouble disco = gamma_1 * gamma_1 - 4 * gamma_2 * gamma_0;
		    
		    if (disco >= 0)
		    {
			t0_gamma_1 = (-gamma_1 - sqrt(disco)) / (2 * gamma_2);
			t0_gamma_2 = (-gamma_1 + sqrt(disco)) / (2 * gamma_2);

			printf("gammas: %.3f,%.3f\n", t0_gamma_1, t0_gamma_2);

			if (sABS(t0_gamma_1 - t0_beta) <= s_EPSILON && sABS(t0_gamma_2 - t0_beta) <= s_EPSILON)
			{
			    //	    if ()
				
			    lower_unsafe_A = t0_beta + kruhobot_traversal_A_linear.m_interval.m_lower;		    
			    upper_unsafe_A = t0_beta + kruhobot_traversal_A_linear.m_interval.m_lower;
		...
		else
		{
			...
			printf("gammas 2: %.3f,%.3f\n", t0_gamma_1, t0_gamma_2);			

			if (sABS(beta_0) <= s_EPSILON)
			{
			    if (gamma_2 > 0)
			    {
				printf("positive\n");
				lower_unsafe_A = t0_gamma_1 + kruhobot_traversal_A_linear.m_interval.m_lower;
				upper_unsafe_A = t0_gamma_2 + kruhobot_traversal_A_linear.m_interval.m_lower;
				...
				Interval avoid_interval_A_low(kruhobot_traversal_A_linear.m_interval.m_lower, sMIN(lower_unsafe_A, upper_unsafe_A));
				Interval avoid_interval_A_high(sMAX(lower_unsafe_A, upper_unsafe_A), kruhobot_traversal_A_linear.m_interval.m_upper);							    
				if (avoid_interval_A_low.size() > s_DELTION)
				{
				    Interval intersection_A_low = kruhobot_traversal_A_linear.m_interval.intersect(avoid_interval_A_low);
				    
				    if (intersection_A_low.size() > s_DELTION)
				    {
					LinearConflict linear_conflict_A(last_conflict_id++, kruhobot_traversal_A_linear.m_u_loc_id, kruhobot_traversal_A_linear.m_v_loc_id, intersection_A_low);
					kruhobot_linear_Conflicts[kruhobot_A_id][Uline(kruhobot_traversal_A_linear.m_u_loc_id, kruhobot_traversal_A_linear.m_v_loc_id)].insert(LinearConflicts_map::value_type(intersection_A_low, linear_conflict_A));
					++affection_A;
					linear_conflict_A.to_Screen();
				    }
				}
				
				if (avoid_interval_A_high.size() > s_DELTION)
				{
				    Interval intersection_A_high = kruhobot_traversal_A_linear.m_interval.intersect(avoid_interval_A_high);
				    
				    if (intersection_A_high.size() > s_DELTION)
				    {
					LinearConflict linear_conflict_A(last_conflict_id++, kruhobot_traversal_A_linear.m_u_loc_id, kruhobot_traversal_A_linear.m_v_loc_id, intersection_A_high);
					kruhobot_linear_Conflicts[kruhobot_A_id][Uline(kruhobot_traversal_A_linear.m_u_loc_id, kruhobot_traversal_A_linear.m_v_loc_id)].insert(LinearConflicts_map::value_type(intersection_A_high, linear_conflict_A));
					++affection_A;
					linear_conflict_A.to_Screen();
				    }				    
				}				
		...
		*/
		...
		if (disc > s_EPSILON)
		{
		    ...
		    lower_unsafe_A = kruhobot_traversal_B_linear.m_interval.m_lower + time0_1;
		    upper_unsafe_A = kruhobot_traversal_B_linear.m_interval.m_lower + time0_2;
		    
		    printf("Unsafe A1: %.3f, %.3f\n", lower_unsafe_A, upper_unsafe_A);		    
			...
			if (intersection_A.size() > s_DELTION)
			{
				...
				printf("Unsafe A2-: %.3f, %.3f\n", T0_1, T0_2);
				printf("Unsafe A2: %.3f, %.3f\n", lower_unsafe_A_, upper_unsafe_A_);
		...
		*/
		...
		if (disc > s_EPSILON)
		{
			...
			if (intersection_B.size() > s_DELTION)
			{
				...
				printf("Unsafe B2: %.3f, %.3f\n", lower_unsafe_B_, upper_unsafe_B_);
